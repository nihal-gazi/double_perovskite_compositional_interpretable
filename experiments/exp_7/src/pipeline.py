...
src/pipeline.py
===============
Pipeline for Experiment 7:
Dual Symbolic Regression Hurdle Architecture (Stage 1 Symbolic Classifier + Stage 2 Symbolic Regressor).
...
def run_exp7_pipeline(
    ...
    log("======================================================================")
    log("EXPERIMENT 7: Dual Symbolic Regression Hurdle Architecture Pipeline")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log("Stage 1: Symbolic Regression Classifier (+1/-1 Classification Fitness)")
    log("Stage 2: Symbolic Regression Non-Zero Regressor (MAE Fitness Metric)")
    log("Operators: add, sub, mul, div, neg, abs, log")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        # ─── STAGE 1: Symbolic Boundary Classifier ───
        if is_zi:
            ...
            log("Running Stage 1 Symbolic Boundary Classifier S_cls(x) > 0...")
        ...
        # ─── STAGE 2: Symbolic Non-Zero Regressor ───
        if is_zi:
            nz_idx = y_all > thr
        else:
        ...
        log(f"\nRunning Stage 2 Symbolic Non-Zero Regressor S_reg(x) on {len(y_nz)} active samples...")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 7: Dual Symbolic Regression Hurdle Architecture Summary Report (Clean Physical Descriptors)\n\n")
        ...
        f.write("**Stage 1 Classifier:** Short Symbolic Regression (`gplearn` custom +1/-1 classification fitness)  \n")
        f.write("**Stage 2 Regressor:** Standard Symbolic Regression (`gplearn` `SymbolicRegressor` with MAE fitness)  \n")
        f.write("**Operator Set:** `['add', 'sub', 'mul', 'div', 'neg', 'abs', 'log']`  \n\n")
        f.write("---\n\n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 7: Discovered Analytical Dual Symbolic Equations Report (Clean Physical Descriptors)\n\n")
        f.write("This document presents the complete 100% interpretable analytical symbolic equations discovered by Genetic Programming on 33 pure physical chemical descriptors (with leaked GNN proxies `E_GNN`, `M_net`, `M_abs` removed).\n\n")
        f.write("---\n\n")
