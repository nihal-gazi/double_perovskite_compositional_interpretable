...
src/pipeline.py
===============
Pipeline for Experiment 8:
Dual Symbolic Regression Hurdle Architecture with 'pow' operator enabled in BOTH Stage 1 Classifier and Stage 2 Regressor.
...
def run_exp8_pipeline(
    ...
    log("======================================================================")
    log("EXPERIMENT 8: Dual Symbolic Regression Hurdle Architecture Pipeline")
    log("Includes 'pow' operator in BOTH Stage 1 Classifier & Stage 2 Regressor")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log("Operators: add, sub, mul, div, neg, abs, log, pow")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        # ─── STAGE 1: Symbolic Boundary Classifier with 'pow' ───
        if is_zi:
            ...
            log("Running Stage 1 Symbolic Boundary Classifier S_cls(x) > 0 (with 'pow')...")
        ...
        # ─── STAGE 2: Symbolic Non-Zero Regressor with 'pow' ───
        if is_zi:
            nz_idx = y_all > thr
        else:
        ...
        log(f"\nRunning Stage 2 Symbolic Non-Zero Regressor S_reg(x) (with 'pow') on {len(y_nz)} active samples...")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 8: Dual Symbolic Regression Hurdle Architecture (with 'pow') Summary Report\n\n")
        ...
        f.write("**Stage 2 Regressor:** Standard Symbolic Regression with `pow` operator (`gplearn` `SymbolicRegressor` with MAE fitness)  \n")
        f.write("**Operator Set:** `['add', 'sub', 'mul', 'div', 'neg', 'abs', 'log', 'pow']`  \n\n")
        f.write("---\n\n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 8: Discovered Analytical Dual Symbolic Equations Report (with 'pow' operator)\n\n")
        f.write("This document presents the complete 100% interpretable analytical symbolic equations discovered by Genetic Programming (using primitive set with `pow`) on 33 pure physical chemical descriptors (with leaked GNN proxies `E_GNN`, `M_net`, `M_abs` removed).\n\n")
        f.write("---\n\n")
