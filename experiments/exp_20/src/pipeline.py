...
src/pipeline.py
===============
Pipeline for Experiment 20:
Single-Perovskite Convex Hull Tie-Line Engine & Exponential Distance Scaling.
...
def run_exp20_pipeline(
    ...
    X_expanded, expanded_feature_cols = generate_exp20_features(df, base_feature_cols)

    print(f"Base Features N = {len(base_feature_cols)} -> Exp 20 Features N = {len(expanded_feature_cols)}")
    ...
    log("======================================================================")
    log("EXPERIMENT 20: Single-Perovskite Convex Hull Tie-Line Engine")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log("Introduces Single-Perovskite Tie-Line Engine (D_hull_proxy) & Exponential Distance Scaling")
    log(f"Expanded Features N = {len(expanded_feature_cols)} (Tie-Line Engine, Exp Scale, d0/d10 Engine, Harrison Gap)")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        X_all, exp_cols = generate_exp20_features(d_clean, base_feature_cols)
        y_all = d_clean[target_col].values

        model = ExponentialHullModel(target_name=target_name)
        ...
        log(f"--> EXPERIMENT 20 RESULTS ({model.arch_type.upper()}):")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 20: Single-Perovskite Convex Hull Tie-Line Engine Report\n\n")
        ...
        f.write("**New Physical Descriptors Added:** Single-Perovskite Competing Phase Tie-Line Engine (`D_hull_proxy`, `Delta_t_sub_perov`, `Delta_H_sub_perov_mismatch`), Exponential Distance Scaling (`Exp_D_hull_scale`).  \n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 20: Discovered Physical Equations Report\n\n")
        f.write("This document presents the pure compositional analytical physical equations discovered across all 4 target properties in Experiment 20.\n\n")
        f.write("---\n\n")
