"""
src/soft_gated_hurdle_model.py
==============================
Engine for Experiment 19:
Soft-Sigmoidal Gated Regressor (For Band Gap) & High-C Hard-Margin Classifier + Stage 2 Regressor.
"""
...
class SoftGatedHurdleModel:
    """
    Soft-Gated Hurdle Model Manager for Exp 19.
    """
    ...
    def __init__(self, target_name: str):
        ...
        if "Formation Energy" in target_name:
            ...
            self.reg_model = Ridge(alpha=1.0, random_state=42)
            # High-C (C=100.0) Hard-Margin Boundary Classifier
            self.cls_model = SVC(kernel='rbf', C=100.0, class_weight='balanced', probability=True, random_state=42)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, threshold: float = None, feature_names: list = None):
        ...
        if self.arch_type == "direct_multi_operator":
            ...
            # Fit Stage 2 Regressor on non-zero subset
            nz_idx = y > threshold
    ...
    def predict(self, X: np.ndarray, threshold: float = None, allow_negative: bool = True) -> tuple[np.ndarray, dict]:
        ...
        if self.arch_type == "direct_multi_operator":
            ...
            probs = self.cls_model.predict_proba(X_scaled)[:, 1] # Smooth probability gate g(x)
            pred_nz = self.reg_model.predict(X_scaled)
            if not allow_negative:
                pred_nz = np.maximum(0.0, pred_nz)

            # Soft-Sigmoidal Gating: Smooth continuous prediction = g(x) * pred_nz
