"""
src/d0_d10_enthalpy_features.py
===============================
Feature engine for Experiment 19:
1. Octahedral d0 / d10 Closed-Shell Crystal Field Engine.
2. Binary Oxide Formation Enthalpy Mismatch Engine.
3. Harrison Quantum Gap, Birch-Murnaghan Strain, Mendeleev, VEC, Mismatches.
"""

import numpy as np
import pandas as pd
from .d0_d10_features_data import (
...
def generate_exp19_features(df: pd.DataFrame, base_feature_cols: list) -> tuple[np.ndarray, list]:
    """
    Constructs Exp 19 features:
    - Octahedral d0/d10 Closed-Shell Engine
    - Binary Oxide Formation Enthalpy Mismatch Engine
    - Harrison Quantum Gap
    - Birch-Murnaghan Strain Engine
    - Pure Compositional & 0D Quantum Features
    """
    feat_df = df[base_feature_cols].copy()
    feat_cols = list(base_feature_cols)

    # 1. Octahedral d0 / d10 Closed-Shell Engine
    ...
    # 3. HOMO-LUMO & Tight-Binding Proxies
    if 'B_site' in df.columns and 'Bprime_site' in df.columns:
    ...
    # 4. Mendeleev Number / Pettifor Scale (M)
    if 'B_site' in df.columns and 'Bprime_site' in df.columns:
    ...
    # 6. Birch-Murnaghan Thermodynamic Strain Proxies
    ...
    # 7. Pure Compositional Mismatch Metrics
    ...
    # 8. Multi-Operator Closed-Shell Interactions
    if 'Is_Closed_Shell_both' in feat_df.columns and 'Harrison_Quantum_Gap_eV' in feat_df.columns:
        feat_df['CS_x_Harrison_Gap'] = feat_df['Is_Closed_Shell_both'] * feat_df['Harrison_Quantum_Gap_eV']
        feat_cols.append('CS_x_Harrison_Gap')

    if 'Delta_H_ox_mismatch' in feat_df.columns and 'Tolerance_Factor' in df.columns:
        feat_df['OxMismatch_x_Tolerance'] = feat_df['Delta_H_ox_mismatch'] * df['Tolerance_Factor']
        feat_cols.append('OxMismatch_x_Tolerance')

    # 9. Sub-linear square root terms
    key_root_cols = ['Tolerance_Factor', 'EN_avg', 'Volume_A3', 'Density_g_cm3', 'Total_HS_FiM', 'Total_d_electrons', 'Harrison_Quantum_Gap_eV', 'E_tolerance_strain', 'Delta_H_ox_mismatch']
    ...
    # 10. Logarithmic scale terms
    key_log_cols = ['Volume_A3', 'Density_g_cm3', 'Total_HS_FiM', 'Total_d_electrons', 'Spin_Proxy_Distance', 'Harrison_Quantum_Gap_eV', 'Delta_Mendeleev_B', 'Delta_H_ox_mismatch']
    ...
    # 11. Squared terms
    key_sq_cols = ['Tolerance_Factor', 'Octahedral_Mismatch', 'EN_avg', 'Total_HS_FiM', 'Volume_A3', 'Density_g_cm3', 'Harrison_Quantum_Gap_eV', 'VEC', 'Delta_H_ox_mismatch']
    ...
    # 12. 2nd-order multiplication interactions
    interaction_pairs = [
    ...
    # 14. 3rd-order physical triplet terms
    triplet_tuples = [
