...
def run_exp19_pipeline(
    ...
    X_expanded, expanded_feature_cols = generate_exp19_features(df, base_feature_cols)

    print(f"Base Features N = {len(base_feature_cols)} -> Exp 19 Features N = {len(expanded_feature_cols)}")
    ...
    log("======================================================================")
    log("EXPERIMENT 19: Octahedral d0/d10 Closed-Shell Engine & Oxidation Enthalpy Mismatch")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log("Introduces d0/d10 Closed-Shell Engine, Binary Oxidation Enthalpy Mismatch & Soft-Gated Regressor")
    log(f"Expanded Features N = {len(expanded_feature_cols)} (d0/d10 Engine, Oxidation Enthalpy, Harrison Gap, Strain)")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        X_all, exp_cols = generate_exp19_features(d_clean, base_feature_cols)
        y_all = d_clean[target_col].values

        model = SoftGatedHurdleModel(target_name=target_name)
        ...
        log(f"--> EXPERIMENT 19 RESULTS ({model.arch_type.upper()}):")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 19: Octahedral d0/d10 Closed-Shell Engine & Oxidation Enthalpy Mismatch Report\n\n")
        ...
        f.write("**New Physical Descriptors Added:** Octahedral $d^0/d^{10}$ Closed-Shell Indicators (`Is_d0_B`, `Is_d10_B`, `Is_Closed_Shell_both`), Binary Oxidation Enthalpy Mismatch ($\\Delta H_{\\text{ox\\_mismatch}}$), Soft-Sigmoidal Gated Regressor.  \n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 19: Discovered Physical Equations Report\n\n")
        f.write("This document presents the pure compositional analytical physical equations discovered across all 4 target properties in Experiment 19.\n\n")
        f.write("---\n\n")
