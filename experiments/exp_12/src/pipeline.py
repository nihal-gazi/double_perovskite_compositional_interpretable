...
src/pipeline.py
===============
Pipeline for Experiment 12:
Physical Interaction & Polynomial-Linear Hybrid Model Baseline across 4 target properties.
Evaluates both Direct Interaction Model and Two-Stage Interaction Hurdle System.
...
def run_exp12_pipeline(
    ...
    log("======================================================================")
    log("EXPERIMENT 12: Physical Interaction & Polynomial-Linear Hybrid Model")
    ...
    log(f"Model Regularization: {model_type.upper()} (alpha = {alpha})")
    log("")
    ...
    for target_name, t_info in target_configs.items():
        ...
        direct_model = InteractionHybridModel(model_type=model_type, alpha=alpha)
        ...
        # ─── 2. TWO-STAGE INTERACTION HURDLE SYSTEM ───
        if is_zi:
            ...
            # Stage 1: Interaction Logistic Classifier
            cls_model = InteractionHybridModel()
            ...
            log("--> STAGE 1 INTERACTION LOGISTIC CLASSIFIER METRICS:")
            log(f"    - Classification Accuracy : {cls_eval['accuracy'] * 100:.2f}%")
            log(f"    - F1-Score               : {cls_eval['f1']:.4f}")

            # Stage 2: Interaction Regressor on Non-Zeros
            nz_idx = y_all > thr
            X_nz = X_all[nz_idx]
            y_nz = y_all[nz_idx]

            hurdle_reg = InteractionHybridModel(model_type=model_type, alpha=alpha)
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 12: Physical Interaction & Polynomial-Linear Hybrid Model Summary Report\n\n")
        ...
        f.write("| Target Property | Direct Interaction R² (%) | **Direct Theoretical Limit (%)** | Stage 1 Cls Acc (%) | Stage 2 Sub R² (%) | **Hurdle Pipeline R² (%)** | **Hurdle Theoretical Limit (%)** | Hurdle MSE | Hurdle MAE |\n")
        f.write("| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |\n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 12: Discovered Physical Interaction Equations Report\n\n")
        f.write("This document contains the top 15 most influential interaction terms ($x_i x_j$, $x_i/x_j$, $x_i^2$) and their fitted coefficients across all 4 target properties.\n\n")
        f.write("---\n\n")
