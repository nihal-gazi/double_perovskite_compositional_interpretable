...
========================
1D Fourier Series Regressor with Ridge Regularization for Numerical Stability at High Depths.
"""
...
class FourierSeriesRegressor:
    """
    Fits 1D Fourier series representation mapping scalar feature x to continuous target y:
    ...
    def _normalize_x(self, x: np.ndarray) -> np.ndarray:
        ...
        # Clip x to training bounds to prevent out-of-domain extrapolation explosion
    ...
    def fit(self, x: np.ndarray, y: np.ndarray):
        ...
        reg_matrix[0, 0] = 0.0  # Do not penalize bias
    ...
    def predict(self, x: np.ndarray) -> np.ndarray:
        ...
        # Clamp predictions to [y_min - buffer, y_max + buffer] for physical safety
