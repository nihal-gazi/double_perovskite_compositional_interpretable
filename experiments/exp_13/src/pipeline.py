...
src/pipeline.py
===============
Pipeline for Experiment 13:
Non-Linear Decision Boundary Hurdle Model across 4 target properties.
Evaluates Direct Interaction Model & Two-Stage Non-Linear Hurdle System.
...
def run_exp13_pipeline(
    dataset_path: str,
    results_dir: str,
    reg_model_type: str = "ridge",
    alpha: float = 1.0,
    cls_type: str = "rbf_svc"
):
    ...
    # Generate Physical Interaction Features
    X_expanded, expanded_feature_cols = generate_physical_interactions(df, base_feature_cols)

    print(f"Base Features N = {len(base_feature_cols)} -> Expanded Interaction Features N = {len(expanded_feature_cols)}")
    ...
    log("======================================================================")
    log("EXPERIMENT 13: Non-Linear Decision Boundary Hurdle Model")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log(f"Expanded Features N = {len(expanded_feature_cols)} (Interactions, Ratios, Squares)")
    log(f"Stage 1 Non-Linear Classifier: {cls_type.upper()} | Stage 2 Regressor: {reg_model_type.upper()}")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        X_all, exp_cols = generate_physical_interactions(d_clean, base_feature_cols)
        y_all = d_clean[target_col].values

        # ─── 1. DIRECT PHYSICAL INTERACTION REGRESSION ───
        direct_model = NonLinearHurdleModel(reg_model_type=reg_model_type, alpha=alpha, cls_type=cls_type)
        ...
        direct_eq = direct_model.format_discovered_equation(target_name, top_k=15)

        log("--> 1. DIRECT INTERACTION MODEL RESULT:")
        log(f"    - Direct R2  : {direct_r2 * 100:.2f}% ({direct_r2:.6f})")
        log(f"    - Direct MSE : {direct_mse:.6f}")
        log(f"    - Direct MAE : {direct_mae:.6f}")

        # ─── 2. TWO-STAGE NON-LINEAR HURDLE SYSTEM ───
        if is_zi:
            ...
            # Stage 1: Non-Linear Classifier (Kernel/RBF SVC or Gradient Boosted Decision Boundary)
            cls_model = NonLinearHurdleModel(cls_type=cls_type)
            ...
            log(f"--> STAGE 1 NON-LINEAR ({cls_type.upper()}) CLASSIFIER METRICS:")
            ...
            # Stage 2: Interaction Regressor on Active Non-Zeros
            nz_idx = y_all > thr
            X_nz = X_all[nz_idx]
            y_nz = y_all[nz_idx]

            hurdle_reg = NonLinearHurdleModel(reg_model_type=reg_model_type, alpha=alpha)
            hurdle_reg.fit_regressor(X_nz, y_nz, feature_names=exp_cols)
            reg_nz_eval = hurdle_reg.evaluate_regressor(X_nz, y_nz)
            hurdle_nz_eq = hurdle_reg.format_discovered_equation(f"{target_name} (Non-Zero)", top_k=15)

            log("--> STAGE 2 INTERACTION REGRESSOR (NON-ZERO SUBSET) METRICS:")
        ...
        log(f"\n>>> COMBINED NON-LINEAR HURDLE SYSTEM RESULT:")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 13: Non-Linear Decision Boundary Hurdle Model Summary Report\n\n")
        f.write("**Dataset:** `exp_v2/data/data_24_7_2026/double_perovskite_dataset.csv` (2,000 double perovskites)  \n")
        f.write("**Data Leakage Audit:** Removed `E_GNN`, `M_net`, `M_abs`. Evaluated on 33 pure physical descriptors expanded with 2nd-order interaction terms ($x_i x_j$, $x_i/x_j$, $x_i^2$).  \n")
        f.write(f"**Stage 1 Decision Boundary Classifier:** Non-Linear {cls_type.upper()} Classifier  \n")
        f.write(f"**Stage 2 Regressor:** Regularized Physical Interaction Regressor ({reg_model_type.upper()})  \n")
        ...
        f.write("| Target Property | Stage 1 Non-Linear Acc (%) | Stage 1 F1 | Stage 2 Sub R² (%) | Direct Interaction R² (%) | **Hurdle Pipeline R² (%)** | **Hurdle Theoretical Limit (%)** | Hurdle MSE | Hurdle MAE |\n")
        f.write("| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |\n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 13: Discovered Physical Interaction Equations Report\n\n")
        f.write("This document contains the top 15 most influential physical interaction terms ($x_i x_j$, $x_i/x_j$, $x_i^2$) and their fitted coefficients across all 4 target properties under the Non-Linear Decision Boundary Hurdle Architecture.\n\n")
        f.write("---\n\n")

        for target_name, eq_dict in fitted_equations.items():
            f.write(f"## Target Property: {target_name}\n\n")
            res = all_results[target_name]
            f.write(f"- **Stage 1 Non-Linear Classification Accuracy:** **{res['cls_acc']*100:.2f}%**  \n")
            f.write(f"- **Stage 2 Non-Zero Sub $R^2$:** **{res['sub_r2']*100:.2f}%**  \n")
            f.write(f"- **Direct Interaction $R^2$:** **{res['direct_r2']*100:.2f}%**  \n")
            ...
            f.write("### 1. Direct Physical Interaction Formula (Top 15 Terms)\n\n")
            f.write("```text\n")
