"""
src/gibbs_hull_features.py
==========================
Feature engine for Experiment 21:
1. Gibbs Phase Separation Energy Engine (Xi_hull_phase_separation, Delta_G_decomp_proxy).
2. Single-Perovskite Convex Hull Tie-Line Engine & Exponential Distance Scaling.
3. Octahedral d0/d10 Closed-Shell Engine, Binary Oxidation Enthalpy, Harrison Gap, Strain.
"""

import numpy as np
import pandas as pd
from .gibbs_hull_data import (
...
def generate_exp21_features(df: pd.DataFrame, base_feature_cols: list) -> tuple[np.ndarray, list]:
    """
    Constructs Exp 21 features:
    - Gibbs Phase Separation Energy Engine (Xi_hull_phase_separation)
    ...
    # 1. Gibbs Phase Separation Energy Engine
    if 'B_site' in df.columns and 'Bprime_site' in df.columns:
        ...
        t_fact = df['Tolerance_Factor'] if 'Tolerance_Factor' in df.columns else 1.0
        oct_mis = df['Octahedral_Mismatch'] if 'Octahedral_Mismatch' in df.columns else 0.1

        delta_h_ox = (h_ox_b - h_ox_bp).abs()
        den = (t_fact ** 2) + (oct_mis ** 2) + 1e-4
        feat_df['Xi_hull_phase_separation'] = delta_h_ox / den
        feat_cols.append('Xi_hull_phase_separation')

        delta_h_sub = (h_ox_b + h_ox_ap - h_ox_bp - h_ox_a).abs()
        ...
        vec = (val_a + val_ap + val_b + val_bp + 36.0) / 10.0

        feat_df['Delta_G_decomp_proxy'] = delta_h_sub - 0.025 * vec
        feat_cols.append('Delta_G_decomp_proxy')

    # 2. Single-Perovskite Convex Hull Tie-Line Engine
    ...
    # 3. Octahedral d0 / d10 Closed-Shell Engine
    ...
    # 4. Binary Oxide Formation Enthalpy Mismatch Engine
    if 'B_site' in df.columns and 'Bprime_site' in df.columns:
        h_ox_b = df['B_site'].map(BINARY_OXIDE_FORMATION_ENTHALPY_EV_ATOM).fillna(-2.0)
        h_ox_bp = df['Bprime_site'].map(BINARY_OXIDE_FORMATION_ENTHALPY_EV_ATOM).fillna(-2.0)
    ...
    # 5. HOMO-LUMO & Tight-Binding Proxies
    if 'B_site' in df.columns and 'Bprime_site' in df.columns:
    ...
    # 6. Mendeleev Number / Pettifor Scale (M)
    if 'B_site' in df.columns and 'Bprime_site' in df.columns:
    ...
    # 7. Valence Electron Concentration (VEC)
    val_a = df['Val_A'] if 'Val_A' in df.columns else 2.0
    val_ap = df['Val_Aprime'] if 'Val_Aprime' in df.columns else 2.0
    val_b = df['Val_B'] if 'Val_B' in df.columns else 3.0
    val_bp = df['Val_Bprime'] if 'Val_Bprime' in df.columns else 3.0
    ...
    # 8. Birch-Murnaghan Thermodynamic Strain Proxies
    ...
    # 9. Pure Compositional Mismatch Metrics
    ...
    # 10. Multi-Operator Gibbs Interactions
    if 'Xi_hull_phase_separation' in feat_df.columns and 'Exp_D_hull_scale' in feat_df.columns:
        feat_df['Xi_x_ExpHull'] = feat_df['Xi_hull_phase_separation'] * feat_df['Exp_D_hull_scale']
        feat_cols.append('Xi_x_ExpHull')

    # 11. Sub-linear square root terms
    key_root_cols = ['Tolerance_Factor', 'EN_avg', 'Volume_A3', 'Density_g_cm3', 'Total_HS_FiM', 'Total_d_electrons', 'Harrison_Quantum_Gap_eV', 'E_tolerance_strain', 'Delta_H_ox_mismatch', 'D_hull_proxy', 'Xi_hull_phase_separation']
    ...
    # 12. Logarithmic scale terms
    key_log_cols = ['Volume_A3', 'Density_g_cm3', 'Total_HS_FiM', 'Total_d_electrons', 'Spin_Proxy_Distance', 'Harrison_Quantum_Gap_eV', 'Delta_Mendeleev_B', 'Delta_H_ox_mismatch', 'D_hull_proxy', 'Xi_hull_phase_separation']
    ...
    # 13. Squared terms
    key_sq_cols = ['Tolerance_Factor', 'Octahedral_Mismatch', 'EN_avg', 'Total_HS_FiM', 'Volume_A3', 'Density_g_cm3', 'Harrison_Quantum_Gap_eV', 'VEC', 'Delta_H_ox_mismatch', 'Delta_t_sub_perov', 'D_hull_proxy', 'Xi_hull_phase_separation']
    ...
    # 14. 2nd-order multiplication interactions
    interaction_pairs = [
        ...
        ('Xi_hull_phase_separation', 'Tolerance_Factor')
    ]
    ...
    ratio_pairs = [
        ...
        ('Xi_hull_phase_separation', 'EN_avg')
    ]
    ...
    # 16. 3rd-order physical triplet terms
    triplet_tuples = [
        ...
        ('Xi_hull_phase_separation', 'E_tolerance_strain', 'Density_g_cm3')
    ]
