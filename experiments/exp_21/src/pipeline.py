...
src/pipeline.py
===============
Pipeline for Experiment 21:
Log-Transformed Target Space & Gibbs Phase Separation Engine.
Evaluates 100% Pure Compositional & 0D Quantum Features.
...
def run_exp21_pipeline(
    ...
    X_expanded, expanded_feature_cols = generate_exp21_features(df, base_feature_cols)

    print(f"Base Features N = {len(base_feature_cols)} -> Exp 21 Features N = {len(expanded_feature_cols)}")
    ...
    log("======================================================================")
    log("EXPERIMENT 21: Log-Transformed Target Space & Gibbs Phase Separation Engine")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log("Introduces Log-Target Space Regression, Gibbs Phase Separation Index (Xi_hull) & High-C Boundary Classifier")
    log(f"Expanded Features N = {len(expanded_feature_cols)} (Gibbs Engine, Log-Target Space, Tie-Line Engine, Harrison Gap)")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        X_all, exp_cols = generate_exp21_features(d_clean, base_feature_cols)
        y_all = d_clean[target_col].values

        model = LogTargetHullModel(target_name=target_name)
        model.fit(X_all, y_all, threshold=thr, feature_names=exp_cols)
        
        pipeline_preds, cls_metrics = model.predict(X_all, threshold=thr, allow_negative=allow_neg)
        ...
        if is_zi and model.cls_model is not None:
            ...
            X_scaled_all = model.scaler.transform(X_all)
            probs = model.cls_model.predict_proba(X_scaled_all)[:, 1]
            ...
            nz_idx = y_all > thr
            X_nz_scaled = X_scaled_all[nz_idx]
            y_nz = y_all[nz_idx]
            
            if model.use_log_target:
                z_nz = np.log(y_nz + 1.0)
                pred_z_nz = model.reg_model.predict(X_nz_scaled)
                pred_y_nz = np.exp(pred_z_nz) - 1.0
                sub_r2 = float(r2_score(y_nz, pred_y_nz))
            else:
                sub_r2 = float(r2_score(y_nz, model.reg_model.predict(X_nz_scaled)))
        else:
            cls_acc = 1.0
            cls_f1 = 1.0
            sub_r2 = master_r2
        ...
        log(f"--> EXPERIMENT 21 RESULTS ({model.arch_type.upper()}):")
        log(f"    - Stage 1 Classification Acc : {cls_acc * 100:.2f}% (F1 = {cls_f1:.4f}, Tau* = {cls_metrics.get('best_tau', 0.5):.4f})")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 21: Log-Transformed Target Space & Gibbs Phase Separation Engine Report\n\n")
        f.write("**Dataset:** `exp_v2/data/data_24_7_2026/double_perovskite_dataset.csv` (2,000 double perovskites)  \n")
        f.write("**Data Leakage & 3D Audit Compliance:** 100% Pure Compositional Descriptors (Zero 3D atomic coordinates, zero GNN proxies `E_GNN`, `M_net`, `M_abs`).  \n")
        f.write("**New Physical Descriptors Added:** Gibbs Phase Separation Energy Index ($\\Xi_{\\text{hull}}$), Entropic Driving Force Proxy ($\\Delta G_{\\text{decomp\\_proxy}}$), Log-Transformed Target Space Regression ($z = \\log(y + 1.0)$).  \n")
        ...
        for target_name, res in all_results.items():
            f.write(f"| **{target_name}** | {res['arch_type']} | {res['cls_acc']*100:.2f}% | {res['sub_r2']*100:.2f}% | **{res['sub_theo_pct']:.2f}%** | **{res['master_r2']*100:.2f}%** | **{res['master_theo_pct']:.2f}%** | {res['master_mse']:.6f} | {res['master_mae']:.4f} |\n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 21: Discovered Physical Equations Report\n\n")
        f.write("This document presents the pure compositional analytical physical equations discovered across all 4 target properties in Experiment 21.\n\n")
        f.write("---\n\n")
