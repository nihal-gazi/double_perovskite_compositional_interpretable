...
def run_exp17_pipeline(
    ...
    X_expanded, expanded_feature_cols = generate_exp17_features(df, base_feature_cols)

    print(f"Base Features N = {len(base_feature_cols)} -> Exp 17 Features N = {len(expanded_feature_cols)}")
    ...
    log("======================================================================")
    log("EXPERIMENT 17: Tight-Binding Proxies & Mendeleev Feature Engine")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log("Introduces HOMO-LUMO Energy Proxies (IE, EA, Delta E_gap), Mendeleev Number (M), VEC")
    log(f"Expanded Features N = {len(expanded_feature_cols)} (Tight-Binding, Mendeleev, VEC, Roots, Logs, Triplets)")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        X_all, exp_cols = generate_exp17_features(d_clean, base_feature_cols)
        y_all = d_clean[target_col].values

        model = CostSensitiveHurdleModel(target_name=target_name)
        ...
        log(f"--> EXPERIMENT 17 RESULTS ({model.arch_type.upper()}):")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 17: Tight-Binding Proxies & Mendeleev Feature Engine Summary Report\n\n")
        ...
        f.write("**New Quantum & Thermodynamic Descriptors Added:** Tight-Binding Gap Proxy ($\\Delta E_{\\text{gap}} = \\min(IE_B, IE_{B'}) - EA_{\\text{O}}$), Mendeleev Scale Number ($\\mathcal{M}$) & Mismatch ($\\Delta \\mathcal{M}_B$), Valence Electron Concentration ($VEC$).  \n")
        f.write("**Algorithmic Upgrade:** Cost-Sensitive Stage 1 Classifier with Class Penalties and Precision-Recall Threshold Tuning ($\\tau^*$).  \n")
        ...
        for target_name, res in all_results.items():
            arch_str = "Direct Multi-Operator" if res['arch_type'] == "direct_multi_operator" else "Cost-Sensitive Hurdle"
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 17: Discovered Tight-Binding & Mendeleev Physical Equations Report\n\n")
        f.write("This document presents the pure compositional analytical physical equations discovered across all 4 target properties in Experiment 17.\n\n")
        f.write("---\n\n")
