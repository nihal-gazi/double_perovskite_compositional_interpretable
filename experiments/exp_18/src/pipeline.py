...
def run_exp18_pipeline(
    ...
    X_expanded, expanded_feature_cols = generate_exp18_features(df, base_feature_cols)

    print(f"Base Features N = {len(base_feature_cols)} -> Exp 18 Features N = {len(expanded_feature_cols)}")
    ...
    log("======================================================================")
    log("EXPERIMENT 18: Quantum Power Laws & Birch-Murnaghan Strain Engine")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log("Introduces Harrison Quantum Gap (E_gap_QM), Birch-Murnaghan Strain, Fractional Powers")
    log(f"Expanded Features N = {len(expanded_feature_cols)} (Harrison QM, Strain Engine, Fractional Powers, Triplets)")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        X_all, exp_cols = generate_exp18_features(d_clean, base_feature_cols)
        y_all = d_clean[target_col].values

        model = HardMarginHurdleModel(target_name=target_name)
        ...
        log(f"--> EXPERIMENT 18 RESULTS ({model.arch_type.upper()}):")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 18: Quantum Power Laws & Birch-Murnaghan Strain Engine Summary Report\n\n")
        ...
        f.write("**New Physical Descriptors Added:** Harrison Quantum Gap ($E_{\\text{gap, QM}} = \\sqrt{\\Delta E_{\\text{gap}}^2 + d_{\\text{ideal}}^{-4}}$), Birch-Murnaghan Strain Proxies ($E_{\\text{tolerance\\_strain}}, E_{\\text{oct\\_distortion\\_strain}}, \\Delta V_{\\text{packing\\_strain}}$), Fractional Power Laws.  \n")
        f.write("**Algorithmic Upgrade:** High-C ($C=50.0$) Hard-Margin Stage 1 Classifier with F1 Threshold Tuning ($\\tau^*$).  \n")
        ...
        for target_name, res in all_results.items():
            arch_str = "Direct Multi-Operator" if res['arch_type'] == "direct_multi_operator" else "Hard-Margin Hurdle"
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 18: Discovered Quantum & Strain Physical Equations Report\n\n")
        f.write("This document presents the pure compositional analytical physical equations discovered across all 4 target properties in Experiment 18.\n\n")
        f.write("---\n\n")
