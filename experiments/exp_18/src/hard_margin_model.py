"""
src/hard_margin_model.py
========================
Engine for Experiment 18:
Hard-Margin Stage 1 Classifier + Multi-Operator Stage 2 Regressor.
Applies a High-C (C=50.0) RBF-Kernel Hard-Margin Boundary with Class Penalties and Precision-Recall Threshold Tuning (tau*).
"""
...
class HardMarginHurdleModel:
    """
    Hard-Margin Hurdle Model Manager.
    Stage 1: High-C Hard-Margin Classifier with Class Penalties & F1 Threshold Tuning (tau*).
    ...
    def __init__(self, target_name: str):
        ...
        if "Formation Energy" in target_name or "Band Gap" in target_name:
            ...
            self.reg_model = Ridge(alpha=1.0, random_state=42)
            # High-C Hard-Margin RBF-Kernel Classifier
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, threshold: float = None, feature_names: list = None):
        ...
        if self.arch_type == "direct_multi_operator":
            ...
            # Hard-Margin Precision-Recall F1 Threshold Optimization
