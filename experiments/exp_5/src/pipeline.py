...
def run_exp5_pipeline(
    ...
    log("======================================================================")
    log("EXPERIMENT 5: Hybrid Symbolic-Fourier Hurdle Architecture Pipeline")
    ...
    for target_name, t_info in target_configs.items():
        ...
        if is_zi:
            ...
            log("Running Symbolic Regression for Decision Boundary S(x) > 0...")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 5: Hybrid Symbolic Decision Boundary Classifier + Optimal Top-K* Fourier Ensemble Regressor Report (Clean Physical Descriptors)\n\n")
        ...
        for target_name, target_dict in all_results.items():
            ...
            f.write("**Discovered Symbolic Decision Boundary Formula S(x):**  \n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 5: Discovered Symbolic & Expanded Fourier Analytical Equations Report (Clean Physical Descriptors)\n\n")
        f.write("This document contains the complete, fully expanded analytical formulas for both **Stage 1 (Symbolic Boundary Classifier)** and **Stage 2 (Fitted 1D Fourier Regressors)** on pure physical chemical descriptors (with leaked GNN proxies `E_GNN`, `M_net`, `M_abs` removed).\n\n")
