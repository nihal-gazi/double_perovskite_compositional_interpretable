...
src/pipeline.py
===============
Pipeline for Experiment 15:
Target-Specific Optimal Interpretable Master Model across all 4 target properties.
Calculates Theoretical Limit (%) relative to literature physical limits.
Generates the Master Discovered Equation Registry.
"""
...
def run_exp15_pipeline(
    ...
    X_expanded, expanded_feature_cols = generate_master_features(df, base_feature_cols)

    print(f"Base Features N = {len(base_feature_cols)} -> Master Multi-Operator Features N = {len(expanded_feature_cols)}")
    ...
    log("======================================================================")
    log("EXPERIMENT 15: Target-Specific Optimal Interpretable Master Model")
    ...
    for target_name, t_info in target_configs.items():
        ...
        log("=" * 80)
        log(f"MASTER TARGET: {target_name}")
        log("=" * 80)

        d_clean = df.dropna(subset=[target_col] + base_feature_cols).copy()
        X_all, exp_cols = generate_master_features(d_clean, base_feature_cols)
        ...
        pipeline_preds, _ = master_model.predict(X_all, threshold=thr, allow_negative=allow_neg)
        ...
        log(f"--> MASTER OPTIMAL MODEL RESULTS ({master_model.arch_type.upper()}):")
        ...
        log(f"    - Final Master Pipeline R2   : {master_r2 * 100:.2f}% ({master_r2:.6f})")
        log(f"    - Master MSE                 : {master_mse:.6f}")
        log(f"    - Master MAE                 : {master_mae:.6f}")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 15: Target-Specific Optimal Interpretable Master Model Report\n\n")
        f.write("**Dataset:** `exp_v2/data/data_24_7_2026/double_perovskite_dataset.csv` (2,000 double perovskites)  \n")
        f.write("**Data Leakage Audit:** Removed `E_GNN`, `M_net`, `M_abs`. Evaluated on 33 pure physical descriptors expanded with multi-operator terms.  \n")
        ...
        f.write("## Master Performance Benchmark Table Across Target Properties\n\n")
        f.write("| Target Property | Master Optimal Architecture | Stage 1 Acc (%) | Stage 2 Sub R² (%) | **Sub R² Limit (%)** | **Final Master R² (%)** | **Master Theoretical Limit (%)** | Master MSE | Master MAE |\n")
        f.write("| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |\n")
    ...
    # Generate discovered_equations.md (Master Equation Registry)
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Experiment 15: Master Registry of Discovered Physical Equations Report\n\n")
        f.write("This document presents the final master registry of optimal analytical physical equations discovered across all 4 target properties.\n\n")
        f.write("---\n\n")
