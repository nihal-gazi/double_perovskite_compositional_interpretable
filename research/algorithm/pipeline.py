...
pipeline.py
===========
Master Execution Pipeline for Capstone Double Perovskite Machine Learning.
Evaluates 100% Pure Compositional & 0D Quantum Features across 4 target properties.
...
def run_master_pipeline(
    ...
    X_expanded, expanded_feature_cols = generate_algorithm_features(df, base_feature_cols)

    print(f"Base Features N = {len(base_feature_cols)} -> Algorithm Features N = {len(expanded_feature_cols)}")
    ...
    log("======================================================================")
    log("MASTER ALGORITHM: CAPSTONE DOUBLE PEROVSKITE MACHINE LEARNING")
    log("Clean Physical Descriptors (No Leakage: E_GNN, M_net, M_abs removed)")
    log("Integrates Tie-Line Engine, Hard-Margin Boundary, Soft-Sigmoidal Gating & Harrison Quantum Gap")
    log(f"Master Features N = {len(expanded_feature_cols)}")
    log("======================================================================")
    ...
    for target_name, t_info in target_configs.items():
        ...
        X_all, exp_cols = generate_algorithm_features(d_clean, base_feature_cols)
        ...
        log(f"--> MASTER ALGORITHM RESULTS ({model.arch_type.upper()}):")
    ...
    with open(md_report_path, "w", encoding="utf-8") as f:
        f.write("# Master Algorithm Execution Report: Capstone Double Perovskite Machine Learning\n\n")
        ...
        f.write("## Master Algorithm Summary Table Across Target Properties\n\n")
    ...
    with open(disc_eq_path, "w", encoding="utf-8") as f:
        f.write("# Master Algorithm: Discovered Analytical Physical Equations Report\n\n")
        f.write("This document presents the pure compositional analytical physical equations discovered across all 4 target properties by the Master Algorithm.\n\n")
        f.write("---\n\n")
