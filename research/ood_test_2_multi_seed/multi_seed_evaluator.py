...
========================
Multi-Seed 80/20 Train-Test Evaluation Harness for Master Capstone Algorithm on 2,000 Dataset.
Runs 80/20 train-test split across 10 random seeds (42 + 9 more: 100, 2026, 777, 999, 101, 102, 103, 104, 105),
...
# 10 random seeds for ood_test_2_multi_seed
SEEDS = [42, 100, 2026, 777, 999, 101, 102, 103, 104, 105]

def run_multi_seed_evaluation_2000(
    ...
    log("======================================================================")
    log(f"MULTI-SEED 80/20 TRAIN-TEST BENCHMARK (2,000 DATASET, {len(SEEDS)} SEEDS)")
    ...
    # Generate Overall Multi-Seed Reports in exp_v2/research/ood_test_2_multi_seed/
    ...
    with open(multi_seed_report_path, "w", encoding="utf-8") as f:
        f.write(f"# Multi-Seed 80/20 Benchmark Report ({len(SEEDS)} Random Seeds, 2,000 Dataset)\n\n")
        f.write(f"**Dataset:** `exp_v2/data/data_24_7_2026/double_perovskite_dataset.csv` (2,000 double perovskites)  \n")
        f.write(f"**Number of Random Seeds Evaluated:** `{len(SEEDS)}` (80% Train = 1,600 samples / 20% Test = 400 samples per seed)  \n")
        ...
        f.write(f"## 10-Seed Statistical Summary Table (Mean ± Standard Deviation)\n\n")
        ...
        f.write("## 10-Seed Breakdown Across Targets\n\n")
    ...
    print(f"\n[SUCCESS] 10-Seed Multi-seed report with Train Limits saved to: {multi_seed_report_path}")
